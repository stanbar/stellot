...
const EncryptedDecrypted: React.FC<EncryptedDecryptedProps> = ({ voting, myTxMemo }) => {
    console.log({ myTxMemo })
    const [decrypted, setDecrypted] = useState<string | undefined>(undefined)
    const [encrypted, setEncrypted] = useState<string | undefined>(myTxMemo.toString('hex'))
            ...
            <AntTooltip title="Ballot is encrypted, click to decrypt">
                <span onClick={() => {
                    ...
                    if (!!targetMyOption && !!myTxMemo) {
                        setEncrypted(myTxMemo.toString('hex'))
