...
const whitelist = [
  'https://stellot.com',
  'https://gh.stellot.com',
  'http://gh.stellot.com',
  'https://stellot.stasbar.com',
  'https://voting.stasbar.com',
  'http://localhost',
];
app.use(cors({ origin: whitelist, exposedHeaders: ['SESSION-TOKEN', 'Content-Type'] }));
...
app.use((err: any, req: Request, res: Response, next: NextFunction) => {
  ...
  if (!isProduction) {
    console.log({ stack: err.stack, data: err.response?.data });
    const errorCode = err?.response?.data?.extras?.result_codes?.transaction;
